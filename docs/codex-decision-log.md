...
`Solve` remains a help request and cannot teach the adapter that the user knows
the generated answer. Only a submitted checkpoint with adequate grader
confidence can update LoRA. Replay-loss regression restores the previous fast
weights. Tests exercise both paths.
...
- Offline corpus, one-epoch training, and evaluation smoke paths execute.
- The production build type-checks; full `next build` additionally needs network
  access to fetch the existing Google font assets.
